...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  result = a > b ? `${b}, ${a}` : `${a}, ${b}`;
  result = isStartIncluded ? `[${result}` : `(${result}`;
  result = isEndIncluded ? `${result}]` : `${result})`;
...
function isBracketsBalanced(str) {
  ...
  const openBr = Object.keys(pairs);
  const stack = [];
  for (let i = 0; i < str.length; i += 1) {
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < f.length; i += 1) {
    if (f[i][0] !== undefined && f[i][0] === f[i][1] && f[i][0] === f[i][2]) {
      return f[i][0];
    }
    if (f[0][i] !== undefined && f[0][i] === f[1][i] && f[0][i] === f[2][i]) {
      return f[0][i];
    }
  }

  if (f[0][0] !== undefined && f[0][0] === f[1][1] && f[0][0] === f[2][2]) {
    return f[0][0];
  }
  if (f[0][2] !== undefined && f[0][2] === f[1][1] && f[0][2] === f[2][0]) {
    return f[0][2];
  }
  return winner;
}
